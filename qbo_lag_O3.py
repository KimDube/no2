...
for i in range(32):
    for j in range(50):
        wavr_oz[i, j] = np.mean([oz1[i, j], oz2[i, j], oz3[i, j], oz4[i, j]])
...
plt.title("OSIRIS O$\mathregular{_3}$")
cb = fig.colorbar(im, orientation='horizontal', fraction=0.2, aspect=50)
cb.set_label("O$\mathregular{_3}$ VMR Anomaly [ppmv]")
plt.tight_layout(rect=[0, -0.1, 1, 1])
plt.savefig("/home/kimberlee/Masters/NO2/Figures/QBO_average_O3.png", format='png', dpi=150)
plt.show()

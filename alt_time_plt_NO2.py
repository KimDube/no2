...
if __name__ == "__main__":
    ...
    monthlystds = tropics.groupby('time.month').std('time')
    anomalies = tropics.groupby('time.month') - monthlymeans
    # anomalies = anomalies.groupby('time.month') / monthlystds

    sns.set(context="talk", style="white", rc={'font.family': [u'serif']})
    fig, ax = plt.subplots(figsize=(10, 5))
    fax = anomalies.plot.contourf(x='time', y='altitude', levels=np.arange(-0.2, 0.2, 0.02), robust=True,
    ...
    plt.xlabel('')
    plt.title('Monthly Mean OSIRIS NO$\mathregular{_2}$ @ 6:30 (-5 to 5 deg. lat.)')
    sns.set(context="talk", style="white", rc={'font.family': [u'serif']})
    ...
    plt.tight_layout()
    plt.savefig("/home/kimberlee/Masters/NO2/Figures/altVtime_NO2_630.png", format='png', dpi=150)
    plt.show()

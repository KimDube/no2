...
if __name__ == "__main__":
    ...
    fax = anomalies.plot.contourf(x='Time', y='nLevels', robust=True, levels=np.arange(-40, 40, 2),
    ...
    plt.xlabel('')
    plt.title('Monthly Mean MLS N$\mathregular{_2}$O (-5 to 5 deg. lat.)')
    sns.set(context="talk", style="white", rc={'font.family': [u'serif']})
    cb = fig.colorbar(fax, orientation='horizontal', fraction=0.2, aspect=50)
    cb.set_label("N$\mathregular{_2}$O Anomaly VMR [ppbv]")
    plt.gca().invert_yaxis()
    plt.tight_layout()
    plt.savefig("/home/kimberlee/Masters/NO2/Figures/altVtime_N2O_MLS.png", format='png', dpi=150)
    plt.show()
